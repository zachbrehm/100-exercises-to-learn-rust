...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        if title.is_empty() {
            panic!("Title cannot be empty, please include a title.")
        }
        if title.len() > 50 {
            panic!("Title cannot be longer than 50 bytes, please use a shorter title.")
        }
        if description.is_empty() {
            panic!("Description cannot be empty, please include a description.")
        }
        if description.len() > 500 {
            panic!("Description cannot be longer than 500 bytes, please use a shorter description.")
        }
